...
mit dem Befehl \texttt{\$ mkdir helpers} ein Unterordner in dem
erzeugten Repository angelegt werden\footnote{Wer diese Dateien nicht herunterladen
...
eingesehen werden. Neben Zeitpunkt, Autor und Beschreibung beinhaltet dies auch
die \textit{Commit-ID} (Abschnitt \ref{sec:commit}).  Die weiteren
...
Inhalt der Dateien werden hier nicht dargestellt (Abschnitt \ref{sec:arch}).
...
letzten Commits (Listing \ref{lst:commit_git-stats}) ist folgende Zeile
enthalten:
...
der den letzten Commit bzw. (\gls{HEAD}) referenziert:
...
(Abbildung \ref{fig:centralworkflow}). Die wesentliche Funktionsweise von Git
ist auf kollaborative Arbeit an \glspl{repository} ausgelegt (Abschnitt
\ref{sec:kernel}).
...
gerichteter azyklischer Graph, bei dem ein Koten durch einen Commit und Kanten
...
nicht von dem Autor selbst in das aktuelle Repository integiert werdeni (Listing
...
bereits existierenden Namen zu erzeugen, dann reagiert Git mit einer
...
hier kein weiterer Objekttyp benötigt. Da der Name des Branches direkt
von der erstellten Referenzdatei abhängig ist, sind auch die Namen von Branches
eindeutig.

Neben der fortgeschrittenen Manipulation von Git Objekten gehen die Autoren
Scott Chacon und Ben Straub in \cite[S.~408-418]{progit} ausführlicherer
...
Arbeitsbereich angesprochen. Mit \textit{\gls{HEAD}} ist neben dem Verweis auf den
neuesten Commit, auch im weitesten Sinne das Repository gemeint. Das \gls{repository} dient
...
Versionskontrollsystemen eine Neuerung dar. Der Index ist ein Bereich
zwischen dem \gls{repository} und dem Arbeitsbereich dar. Er dient dazu, den
nächsten \textit{\gls{HEAD}} bzw. Commit vorzubereiten. Die Befehle \texttt{\$ git
...
Tree referenzieren. \cite[52]{gitosp} So benötigt \texttt{\$ git ls-tree} aus
Abschnitt \ref{sec:objectmodel} ein \textit{tree-ish} und \texttt{\$ git show}
kann auf allen Objekten aus Abschnitt \ref{sec:objectmodel} arbeiten.
...
aus den Prüfsummen eine Historie hervor. Git ermöglicht mit dem Befehl
...
fortlaufende Zahlen. Um den Tag zusätzlich zu identifizieren, kann hier auch
...
wählt Git einfach den aktuellen \texttt{HEAD}. Das folgende Beispiel erstellt
einen Branch namens \textit{topic} von der Referenz \textit{master}.
...
. So kann mit den Parametern
\texttt{-d/-D}\footnote{Der Parameter \texttt{-D} wird benötigt, wenn auf dem
...
ist das Standardverhalten, wenn kein Parameter übergeben wird.} angezeigt
...
sowie beispielsweise \texttt{\$ git diff} oder \texttt{\$ git show} (Abschnitt
\ref{sec:first_commits}). Die Auswahl des angezeigten Bereichs erfolgt immer
exklusive des ersten und inklusive des zweites (nach \texttt{..}) angegebenen
...
nur im Arbeitsbereich (Abbildung \ref{fig:trees}) oder bereits auf dem Index
vorliegen (Abschnitt \ref{sec:gitadd}). Um diese Änderungen anzuzeigen, kann
\texttt{\$ git diff} verwendet werden. Wird der letzte Absatz aus der
Lizenzdatei des verwendeten Beispiels (Abschnitt \ref{sec:first_commits})
entfernt, zeigt der Befehl \texttt{\$ git status} folgende Ausgangsituation:
...
Index (Listing \ref{lst:git_status_staged}) und das nochmalige Ausführen von
\texttt{\$ git diff}, führt zu einer leeren Ausgabe.

\eqlst{listings/git_status_staged.lst}{lst:git_status_staged}{Status des Index anzeigen}

Um die Änderungen auf dem Index anzuzeigen, muss der Befehl \texttt{\$ git
...
werden.}\textsuperscript{\footnotesize{,}}\footnote{Der Parameter
