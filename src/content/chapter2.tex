...
Eigenschaft ist, dass verschiedene Teams das gleichzeitige Arbeiten an
...
bei Bell Labs geschrieben\footnote{\url{http://www.belllabs.com/}}
...
erzeugten Verzeichnissen durchgeführt werden, sind diese ledigich Zeiger auf
eine Menge von Objekte die mir der Kopie der Quelle übereinstimmen. Die
...
Verzeichnis. Alle Verzeichnisse können in der gleichen Art und Weise bearbeitet
werden. Ein großer Vorteil von \acrshort{svn} gegenüber vorherigen
...
Kommando an das zentrale Repository gesendet. Des Weiteren seien sogenannte
Externals erwähnt, die es als weitere Funktionalität ermöglichen, erstmals
Inhalte von anderen Repositorys einzubinden. Dies kann nützlich sein, um
Abhängigkeiten beispielsweise zwischen \glspl{repository}
abzubilden und Binärdateien auszulagern. \acrshort{svn} stellt zwar eine
erhebliche Verbesserung gegenüber \acrshort{cvs} dar, hatt aber aufgrund der
zusätzlichen Client-/Server Komponente einige neue Probleme. So sind zwar
die Aktionen auf Seiten des zentralen \glspl{repository} atomar, aber nicht
auf Seiten des Clients so dass, hier bei unvorhergesehenen Fehlern wieder
Inkonsistenzen entstehen können. Des Weiteren verwaltet \acrshort{svn} mit
Hilfe eines Unterordners \textit{.svn} in jedem enthaltenen Verzeichnis die
Verwaltungsinformationen desselbigen. Das ermöglicht eine unabhängige
Verwaltung aller Verzeichnisse untereinander, führt aber ggf. dazu, dass die
lokale Kopie eine Summe aus unterschiedlichen Revisionen verschiedener
Unterordner ist und keinen eindeutigen Versionsstand repräsentiert. Weitere
Vor- und Nachteile von SVN sind beispielsweise in \cite[S.~383-385]{cd}
beschrieben.
...
System BitKeeper für die Entwicklung des Linux
...
sich 2005 azu, ein neues \gls{vcs:de} zu schreiben. Dieses sollte weit mehr als
eine Alternative zu den bisherigen sein \cite[S.~13]{gitosp}. So lautete
...
kämpfen hatten, sollte das zentrale Augenmerk auf Geschwindigkeit und
Integrität liegen \cite[S.~13]{gitosp}.
...
erfolgreiche Migration zu und Verwaltung des Linux Kernel \glspl{repository}
...
durch eine solche Zusammenarbeit enstehen können. Der Einsatz eines
Versionsverwaltungssystems sollte als eine gute Vorgehensweise angesehen werden,
...
gefördert werden sollen. Hierzu zählen nach Jennifer Davis und Katherine
Daniels \cite[S.~178]{effdo} beispielsweise:
...
Unter Git sind Tags lediglich symbolische Namen für Referenzen zu einzelnen
Commits. Es gibt hier zwei verschiedene Arten von Commits. Zum Einen
leichtgewichtige Tags (\textit{lightweight tags}), die keine weiteren
...
oder ggf. eine GPG-Signatur. \cite[S.~21]{gitosp}
